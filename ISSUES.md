...
- [x] [GN-206] Develop a system that reloads all JS/CSS/HTML when a new version is released. Today, we are hosted on GitHub and the new version is probably the new code. Find a way to detect when the code changed and reload it if a browser holds an older version of code.
  - Version manifest mismatches now pass the remote build into the reload path, clear Cache Storage, and navigate with a `build` query so JS/CSS assets refetch on deploy; `index.html` tags include the same fingerprint to bust long-lived caches.
- [x] [GN-207] The icon for the full screen shall be placed under the avatar icon with Full Screen text as a menu item. The icon of the exiting screen shall be changed to a diagonal like with two slapp ticks, 90 degrees opening out.
  - Moved the toggle into the avatar menu with a labeled control, refreshed the exit icon styling, and updated regression coverage for the new location.
- [x] [GN-208] Only display user's name, not email, under the avatar
  - Removed the email field from the profile header and simplified auth controls so dropdowns now surface only the display name.
...
- [ ] [GN-20] Add a command to Makefile to build and launch local docker image through docker-compose.yml
