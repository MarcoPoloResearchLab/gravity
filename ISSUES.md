...
- [ ] [GN-201] Eliminate remaining shared/global state across the frontend/test harness (for example, globalThis runtime context caches and debug flags). Replace with explicit dependency injection or per-instance state so runtime config and test harnesses do not rely on globals.
