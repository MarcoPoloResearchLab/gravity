...
- [x] [GN-314] Inline editor regressions after the note-search overlay
  - Search overlay now measures against the EasyMDE container/CodeMirror bounds, explicitly sizes the toolbar layer, and keeps the inline padding offset in sync so navigation buttons stay clickable.
  - Pointer tracking already covered `.editor-search`; retained compatibility while keeping the overlay inside the editor host and refreshing metrics on search queries.
  - `editor.search.puppeteer.test.js` passes; `editor.inline.puppeteer.test.js` still hits the pre-existing focus test failure (double-click caret drift). Full `npm test` run continues to be constrained by the harness timeout, so only the focused suites could be validated end-to-end.
- [x] [GN-310] I had an expanded HTML view. I clicked on a checkmark. It has folded the expanded view and moved the html view to the top in its exanded view. What I was expecting: the checkmark becoming checked and no other movements on the screen. Ensure that checking on a checkmar in HTML rendered mode does not perform any immediate repositioniing of the card. The card visually stays where it is. It's markdown has changed and the other cards have changed their positions the active card does not move  
  - Added `expanded htmlView checkbox toggle keeps the card anchored in place` Puppeteer coverage and skipped the delayed `bubbleCardToTop` scheduling whenever the card is expanded so checkbox toggles preserve both expansion and grid position.
