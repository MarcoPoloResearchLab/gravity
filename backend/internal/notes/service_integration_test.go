...
func (generator failingIDGenerator) NewID() (string, error) {
	return "", generator.errorToReturn
}
...
func TestServiceAppliesNewUpsert(t *testing.T) {
	...
	changes := []ChangeEnvelope{
		mustEnvelope(t, ChangeEnvelopeConfig{
			...
			PayloadJSON:     `{"noteId":"note-1","markdownText":"hello"}`,
		}),
	}
...
func TestServiceRejectsStaleBaseVersion(t *testing.T) {
	...
	existing := Note{
		...
		PayloadJSON:       `{"noteId":"note-1","markdownText":"existing"}`,
	...
	if stored.PayloadJSON != `{"noteId":"note-1","markdownText":"existing"}` {
...
func TestServiceRollsBackOnIDGenerationFailure(t *testing.T) {
	idProviderError := errors.New("id generation failed")
	service, db := newTestServiceWithIDProvider(t, failingIDGenerator{errorToReturn: idProviderError})
	userID := mustUserID(t, "user-1")
	noteID := mustNoteID(t, "note-1")
	...
	changes := []ChangeEnvelope{
		mustEnvelope(t, ChangeEnvelopeConfig{
			...
			ClientDevice:    "web",
			Operation:       OperationTypeUpsert,
			PayloadJSON:     `{"noteId":"note-1","markdownText":"rollback"}`,
		}),
	}

	_, err := service.ApplyChanges(context.Background(), userID, changes)
	if err == nil {
		t.Fatal("expected error from id generation failure")
	}
	var serviceErr *ServiceError
	if !errors.As(err, &serviceErr) {
		t.Fatalf("expected ServiceError, got %T", err)
	}
	if serviceErr.Code() != "notes.apply_changes.id_generation_failed" {
	...
	if noteCount != 0 {
		t.Fatalf("expected no notes after rollback, got %d", noteCount)
	}
	...
	if auditCount != 0 {
		t.Fatalf("expected no audits after rollback, got %d", auditCount)
	}
}

func newTestService(t *testing.T, ids []string) (*Service, *gorm.DB) {
	generator := &staticIDGenerator{ids: ids}
	return newTestServiceWithIDProvider(t, generator)
}

func newTestServiceWithIDProvider(t *testing.T, idProvider IDProvider) (*Service, *gorm.DB) {
	t.Helper()
