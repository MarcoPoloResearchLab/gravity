...
test("run-tests harness surfaces timeouts in summary", async () => {
  ...
  const snapshot = await readContextSnapshot();
  try {
    const pattern = `^${escapeRegExp(fixtureRelative)}$`;
    const result = await runHarnessWithOptions({
      pattern,
    ...
    // Harness must exit non-zero when a timeout occurs
    assert.notEqual(result.exitCode, 0, "harness should surface timeout failures");

    const plain = stripAnsi(result.stdout);
    assert.match(plain, new RegExp(escapeRegExp(fixtureRelative), "u"), "summary lists the fixture");
    assert.match(plain, /timeout/i, "summary mentions timeout");

    // terminationReason may differ depending on platform, but must be one of the expected failure reasons
    const allowed = new Set([
      harnessDefaults.terminationReason.exit,
      harnessDefaults.terminationReason.signal,
